...
export const loginUser = createAsyncThunk("user/loginUser", async({ email, password }, { rejectWithValue }) => {
    try {
        ...
        if (!response.ok) {
            throw new Error("Gagal login, pastikan E-Mail dan Password anda sudah benar.");
        }
...
export const registerUser = createAsyncThunk("user/registerUser", async({ name, role, email, password, password_confirmation }, { getState, rejectWithValue }) => {
    try {
        const { user } = getState();
...
export const resetPassword = createAsyncThunk("user/resetPassword", async({ token, email, password, password_confirmation }, { rejectWithValue }) => {
    try {
...
export const updateUser = createAsyncThunk("user/updateUser", async({ id, name, email }, { getState, rejectWithValue }) => {
    try {
        const { user } = getState();
        const { userToken } = user;
...
export const deleteUserById = createAsyncThunk("user/deleteUserById", async(id, { getState, rejectWithValue }) => {
    try {
        const { user } = getState();
        const { userToken } = user;
        const response = await fetch(URL_API + "user/" + id, {
