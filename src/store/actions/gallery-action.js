...
export const getAllGallery = createAsyncThunk("gallery/getAllGallery", async (url = URL_API, { rejectWithValue }) => {
  try {
...
export const createGallery = createAsyncThunk("gallery/createGallery", async ({ title, image }, { getState, rejectWithValue }) => {
  try {
    const { user } = getState();
...
export const deleteGalleryById = createAsyncThunk("gallery/deleteGalleryById", async (id, { getState, rejectWithValue }) => {
  try {
    const { user } = getState();
    const { userToken } = user;
