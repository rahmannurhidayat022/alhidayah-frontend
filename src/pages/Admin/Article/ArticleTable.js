...
import { deleteArticleById, getArticles } from "../../../store/actions/article-action";
import { showAlert } from "../../../store/slices/ui-slice";

const ArticleTable = () => {
  const dispatch = useDispatch();
  const { items, success, error, pagination } = useSelector((state) => state.article);
        ...
        return (
          <tr key={index}>
            ...
            <td className="border border-indigo-300 p-2">
              <a className="underline text-blue-500" target="_blank" href={imageUrl} rel="noreferrer">
                {item?.image}
              </a>
            </td>
            ...
            <td className="border border-indigo-300 border-b-0 p-2 flex flex-row flex-nowrap space-x-1 justify-center items-stretch">
              <Link to={`/artikel/view/${item.id}`} className="p-3 bg-gray-300 rounded">
                <AiOutlineEye size={20} />
              </Link>
              <Link to={`/artikel/update/${item.id}`} className="p-3 bg-orange-300 rounded">
                <AiOutlineEdit size={20} />
              </Link>
              <button onClick={() => dispatch(deleteArticleById(item.id))} type="button" className="p-3 bg-red-300 rounded">
                <BiTrashAlt size={20} />
              </button>
            </td>
          </tr>
  ...
  return (
    <section className="p-4 rounded bg-white">
      <h2 className="mb-3 font-semibold text-xl underline underline-offset-8 text-indigo-900">Tabel Artikel</h2>
      <div className="mb-3 py-2 inline-flex flex-nowrap overflow-x-auto">
        <Link className="px-3 py-2 text-[16px] bg-indigo-900 rounded text-white" to="/artikel/add">
          Tambah
        </Link>
      </div>
