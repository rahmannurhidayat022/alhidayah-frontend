...
import { deleteGalleryById, getAllGallery } from "../../../store/actions/gallery-action.js";
...
const GalleryTable = () => {
  ...
  const { items, success, error, pagination } = useSelector((state) => state.gallery);
        ...
        return (
          <tr key={index}>
            ...
            <td className="border border-indigo-300 border-b-0 p-2 flex flex-row flex-nowrap space-x-1 justify-center items-stretch">
              ...
              <button onClick={() => dispatch(deleteGalleryById(item?.id))} type="button" className="p-3 bg-red-300 rounded">
                <BiTrashAlt size={20} />
              </button>
            </td>
          </tr>
  ...
  return (
    <section className="p-4 rounded bg-white">
      <h2 className="mb-3 font-semibold text-xl underline underline-offset-8 text-indigo-900">Tabel Dokumentasi Poto</h2>
      <div className="mb-3 py-2 inline-flex flex-nowrap overflow-x-auto">
        <Link className="px-3 py-2 text-[16px] bg-indigo-900 rounded text-white" to="/gallery/add">
          Tambah
        </Link>
      </div>
