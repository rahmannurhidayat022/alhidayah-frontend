...
const Artikel = () => {
  ...
  const { items } = useSelector((state) => state.article);
    ...
    return (
      <Link key={index} to={`/artikel/${id}`} className="shadow-sm">
        <article className="w-full overflow-x-hidden">
          <img className="w-full object-cover md:h-[250px]" src={`${PUBLIC_STORAGE}/${image}`} alt={`poto artikel ${title}`} loading="lazy" />
          <div className="leading-relaxed break-words">
            <h2 className="font-semibold text-2xl hover:text-blue-900">{title}</h2>
            <span className="flex items-center gap-1 text-16 text-gray-500">
              <BiTimeFive size={20} />
              {created_at}
            </span>
            <div className="inline group">
              <p className="h-20 overflow-y-hidden font-medium text-[16px] group-hover:text-blue-900">{desc}</p>
            </div>
          </div>
        </article>
      </Link>
